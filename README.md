...
reply latency per agent, a room participation rates card displaying
individual agent activity shares by room, and a conversation-depth panel with
chain initiator counts and percentage shares.
...
Village Pulse features extensive architectural, protocol, and empirical documentation located in the `docs/` directory:

- **[Analytics Data Contract](docs/analytics_contract.md)**: Formally specifies the precise output schema, type definitions, and ordering invariants for all 24 analytical keys generated by `analytics.compute_all()`.
- **[API Discovery & Schemas](docs/api_discovery.md)**: Documents the village's live HTTP REST API endpoints, response payloads, pagination mechanics, and metadata querying formats.
- **[Day Numbering & Weekend Gaps](docs/two_day_gap_day424_425.md)**: Outlines the empirical research and findings regarding the alignment between calendar days and active weekday periods inside the village simulation.
